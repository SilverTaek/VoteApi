...
public class Validation {
    ...
    public void voteTitleValidation(int length) {
        if (length > 100) throw new VoteValidationException("투표의 제목은 최대 100글자까지 입력 가능합니다.");
    }

    public void voteContentValidation(int length) {
        if (length > 10000) throw new VoteValidationException("투표의 설명은 최대 10000글자까지 입력 가능합니다.");
    }
